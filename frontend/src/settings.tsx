...
import {ArrowLeft, ArrowUpRight, Check, CheckCircle2, ChevronDown, Cpu, FolderOpen, HardDrive, Info, LoaderCircle, Network, Plug, RefreshCw, RotateCcw, Save, Settings2} from 'lucide-react';
import './settings.css';

export type SettingsSection = 'network'|'engines'|'model'|'library'|'comfyui';
...
const sections = [
  {id:'network',label:'Network',detail:'Access from your LAN',icon:Network},
  {id:'engines',label:'Engines',detail:'Locations & versions',icon:Cpu},
  {id:'model',label:'Model setup',detail:'Memory & responses',icon:Settings2},
...
export function ApplicationSettings({visible,section,onSection,request,copy,modelSettings,modelName,onBack,onRescan,busy,modelLoaded,onRestart}:{visible:boolean;section:SettingsSection;onSection:(section:SettingsSection)=>void;request:(path:string,body?:unknown,method?:string)=>Promise<any>;copy:(value:string)=>Promise<void>;modelSettings:React.ReactNode;modelName?:string;onBack:()=>void;onRescan:()=>Promise<void>;busy:boolean;modelLoaded:boolean;onRestart:()=>void}) {
  ...
  return <main className="preferences-page" aria-label="Inflect settings">
    <div className="preferences-heading"><div><span className="eyebrow">MAKE IT YOURS</span><h1>Settings</h1><p>A few thoughtful controls. Everything stays on your computer.</p></div><button className="secondary" onClick={onBack}><ArrowLeft size={15}/>Back to chat</button></div>
    <div className="preferences-layout"><nav className="preferences-nav" aria-label="Settings sections">{sections.map(({id,label,detail,icon:Icon})=><button key={id} aria-current={section===id?'page':undefined} onClick={()=>onSection(id)}><Icon size={18}/><span><strong>{label}</strong><small>{detail}</small></span></button>)}<p><HardDrive size={13}/>Saved on this computer</p></nav>
      <div className="preferences-content"><div className="preferences-section-title"><meta.icon size={21}/><div><h2>{meta.label}</h2><p>{section==='model'?modelName||'Choose a model in the library':meta.detail}</p></div></div>
        ...
        {section==='network'&&<>
          <Card title="Connect from your LAN" description="Use Inflect from another computer, tablet, or phone on the same private network.">
            ...
            <p className="settings-help"><Info size={15}/>Use a trusted LAN: devices on the allowed subnet can use your chats and controls. Inflect does not open router ports or listen on public addresses.</p>
            <details className="preferences-advanced"><summary>Connection details<ChevronDown size={15}/></summary><div className="preferences-grid"><label className="field">Network interface<select disabled={locked||!draft.network_enabled} value={draft.network_interface} onChange={e=>edit('network_interface',e.target.value)}><option value="">Automatic · recommended</option>{snapshot.interfaces.map(item=><option key={item.name+item.host} value={item.name}>{item.name} · {item.host}</option>)}{draft.network_interface&&!snapshot.interfaces.some(item=>item.name===draft.network_interface)&&<option value={draft.network_interface}>{draft.network_interface} · unavailable</option>}</select><small>Detected from your private LAN connection.</small></label><label className="field">Port<input type="number" min={1024} max={65535} value={draft.port} disabled={locked} onChange={e=>edit('port',Number(e.target.value))}/><small>Keep 7860 unless another app uses it.</small></label></div><p className="settings-help">Currently serving: {snapshot.active.url} · {snapshot.active.lan_network||'this computer only'}. Network changes need a restart.</p></details>
          </Card>
        </>}
        {section==='engines'&&<>
          ...
          {(busy||modelLoaded)&&<p className="settings-help"><Info size={15}/>Unload the model in Model setup before updating an engine.</p>}
        ...
        {section==='model'&&<div className="preferences-model"><p className="settings-help">These controls apply to the selected model. Save a profile to keep a setup for next time.</p>{modelSettings}</div>}
        {section==='library'&&<>
          <Card title="Your model folder" description="Inflect discovers compatible models in this folder and its subfolders.">{pathField('model_root','Model folder','A folder on the computer running Inflect. A new location needs a restart.')}<div className="preferences-row"><p className="settings-help">Added new models to the current folder?</p><button className="secondary" disabled={locked||dirty} onClick={()=>act(async()=>{await onRescan();setNotice('Model library rescanned.')})}><RefreshCw size={14}/>Rescan library</button></div></Card>

...
import {ArrowDown,ArrowUpRight,Check,ChevronDown,Copy,Download,FileText,GitBranch,History,LoaderCircle,Maximize2,MessageSquare,Minimize2,MoreHorizontal,Paperclip,Pencil,Plus,RotateCcw,Search,Settings2,Sparkles,Square,Trash2,Upload,X} from 'lucide-react';
...
type Props={model:Model;models:Model[];settings:Settings;session:any;active:boolean;dirty:boolean;expanded:boolean;onExpand:()=>void;onModel:(model:Model)=>void;onBusy:(busy:boolean)=>void;onPreview:(open:boolean)=>void;onSaveProfile:()=>void;onLoad:()=>void;onSettings:()=>void};
export function ChatWorkspace({model,models,settings,session,active,dirty,expanded,onExpand,onModel,onBusy,onPreview,onSaveProfile,onLoad,onSettings}:Props) {
  ...
  useEffect(()=>{const el=composer.current;if(el){el.style.height='auto';el.style.height=Math.min(el.scrollHeight,180)+'px'}},[draft]);
  ...
  return <section className={`chat-workspace ${historyOpen?'history-visible':''} ${preview?'preview-visible':''}`} aria-label="Chat workspace">
    ...
    <div className="chat-center" onPaste={e=>{const files=Array.from(e.clipboardData.files);if(files.length&&!blocked){e.preventDefault();attach(files)}}} onDragOver={e=>{if(e.dataTransfer.types.includes('Files')){e.preventDefault();setDragging(true)}}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget as Node))setDragging(false)}} onDrop={e=>{e.preventDefault();setDragging(false);if(!blocked)attach(Array.from(e.dataTransfer.files))}}>
      ...
      {loading?<div className="chat-welcome"><LoaderCircle className="spin"/><h2>Opening your conversations…</h2></div>:!doc?<div className="chat-welcome"><h2>Let’s reconnect.</h2><p>Your chat library could not be opened.</p><button className="primary" onClick={()=>window.location.reload()}>Try again</button></div>:doc.messages.length===0?<div className="chat-welcome"><InflectMark size={58}/><span className="eyebrow">ROOM FOR YOUR NEXT IDEA</span><h2>Where shall we begin?</h2><p>A question, a rough idea, something worth building.<br/>Make it take shape with Inflect.</p><div className="chat-starters"><button onClick={()=>{changeDraft('Create a beautiful, self-contained HTML dashboard with interactive charts using only HTML, CSS and JavaScript. Put the complete file in one html code block.');composer.current?.focus()}}><span><Sparkles size={17}/></span><strong>Build something</strong><small>Turn an idea into a working preview</small><ArrowUpRight size={15}/></button><button onClick={()=>fileInput.current?.click()}><span><Paperclip size={17}/></span><strong>Look a little closer</strong><small>Explore a document, image or code</small><ArrowUpRight size={15}/></button></div><small className="welcome-local"><span className="local-indicator"/>Your conversations stay on this computer</small></div>:<div className="messages inflect-messages">{doc.messages.map((message,index)=><article key={message.id} className={`message ${message.role} ${message.error?'failed':''}`}><div className="message-avatar">{message.role==='assistant'?<InflectMark size={30}/>:<span>You</span>}</div><div className="message-body"><div className="message-author">{message.role==='assistant'?'Inflect':'You'}{message.role==='assistant'&&<span>{message.model}</span>}<time dateTime={new Date(message.created_at*1000).toISOString()}>{new Date(message.created_at*1000).toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit'})}</time></div>{message.attachments.length>0&&<div className="message-files">{message.attachments.map(id=>fileChip(id))}</div>}{message.reasoning&&<details className="reasoning"><summary><Sparkles size={13}/>Thinking</summary><div className="markdown"><RichMarkdown text={message.reasoning} onPreview={openPreview}/></div></details>}<div className="markdown"><RichMarkdown text={message.content} onPreview={openPreview}/></div>{streaming&&index===doc.messages.length-1&&<span className="typing" aria-label="Generating"><i/><i/><i/></span>}{message.cancelled&&<p className="response-state"><Square size={11}/>Stopped · partial answer saved</p>}{message.error&&<p className="response-state error">{message.error_message||'This reply did not complete. You can retry it.'}</p>}{message.metrics?.finish_reason==='length'&&<p className="output-limit-note">Output limit reached. Increase Maximum output in model settings, then retry.</p>}{message.metrics&&<Performance metrics={message.metrics} compact/>}<div className="message-actions"><button title="Copy message" aria-label="Copy message" onClick={()=>copy(message)}>{copied===message.id?<Check size={13}/>:<Copy size={13}/>}<span>{copied===message.id?'Copied':'Copy'}</span></button>{message.role==='user'?<button disabled={blocked} onClick={()=>setEdit({index,text:message.content})}><Pencil size={13}/>Edit</button>:<button disabled={blocked||!active||!!dirty} onClick={()=>retry(index)}><RotateCcw size={13}/>Retry</button>}<button disabled={busy} onClick={()=>branch(index)} title="Continue from here in a new conversation"><GitBranch size={13}/>Branch</button></div></div></article>)}</div>}
      </div></div>{showLatest&&<button className="jump-latest" onClick={latest}><ArrowDown size={14}/>Jump to latest</button>}</div>
      <div className="inflect-composer-wrap"><div className="message-options"><label><Sparkles size={13}/>Thinking<ThinkingSelect model={model} value={effort} disabled={blocked} onChange={setEffort} label="Chat thinking level"/></label><button className="composer-settings" onClick={onSettings}><Settings2 size={13}/>Model settings</button></div>{draftFiles.length>0&&<div className="draft-files">{draftFiles.map(id=>fileChip(id,true))}</div>}<div className="composer"><textarea ref={composer} aria-label="Message" placeholder={active?'Ask anything, or bring a file…':'Draft a message…'} disabled={busy||!doc} value={draft} onChange={e=>changeDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();if(!blocked)send()}}} rows={2}/><div className="composer-toolbar"><button className="icon-btn" disabled={busy||!doc} aria-label="Attach files" title="Attach files · HTML, PDF, images, Markdown, code or audio" onClick={()=>fileInput.current?.click()}><Paperclip size={18}/></button><span>Files & previews</span><div className="toolbar-spacer"/><small>Shift + Enter for a new line</small>{streaming||doc?.generating?<button className="send-button" aria-label="Stop generation" onClick={stop}><Square size={16}/></button>:<button className="send-button" aria-label="Send message" disabled={!active||!!dirty||blocked||!doc||(!draft.trim()&&!draftFiles.length)} onClick={()=>send()}><ArrowUpRight size={20}/></button>}</div></div><div className="context-footer"><span>{doc?.used_context!=null?`${formatNumber(doc.used_context,0)} / ${formatNumber(settings.context,0)} tokens`:`${formatNumber(settings.context,0)} token context`}</span><span>{doc?.system_prompt?'Custom instructions · ':''}{active?'Connected locally':'Model not loaded'}</span></div></div>
    ...
    {modal==='model'&&<Modal title="Choose a model" onClose={()=>setModal(null)} className="model-picker"><p className="dialog-description">Your conversation stays open when you switch models. Load your selection to continue.</p><div className="model-picker-list">{models.map(m=><button key={m.id} className={m.id===model.id?'selected':''} onClick={()=>{onModel(m);setModal(null)}}><span><strong>{m.title}</strong><small>{m.format} · {m.quant} · {m.vision?'Vision':'Text'}</small></span>{m.id===model.id&&<Check size={16}/>}</button>)}</div></Modal>}

...
type Props={model:Model;models:Model[];settings:Settings;session:any;active:boolean;dirty:boolean;expanded:boolean;onExpand:()=>void;onModel:(model:Model)=>void;onBusy:(busy:boolean)=>void;onPreview:(open:boolean)=>void;onSaveProfile:()=>void;onLoad:()=>void;onSettings:()=>void;profiles?:Profile[];onProfile?:(p:Profile)=>void;setupLabel?:(p:Profile)=>string};
export function ChatWorkspace({model,models,settings,session,active,dirty,expanded,onExpand,onModel,onBusy,onPreview,onSaveProfile,onLoad,onSettings,profiles=[],onProfile,setupLabel}:Props) {

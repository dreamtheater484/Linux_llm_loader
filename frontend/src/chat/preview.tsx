...
import {Code2,Download,Eye,FileText,Maximize2,Minimize2,RotateCcw,X} from 'lucide-react';
...
export function sandboxDocument(text:string,kind:string) {
  const policy="default-src 'none'; script-src 'unsafe-inline' 'unsafe-eval' data: blob:; style-src 'unsafe-inline'; img-src data: blob:; font-src data:; media-src data: blob:; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'";
...
export function PreviewPanel({preview,onClose}:{preview:Preview;onClose:()=>void}) {
  const [mode,setMode]=useState<'preview'|'source'>('preview'),[text,setText]=useState(preview.text||''),[error,setError]=useState(''),[wide,setWide]=useState(false),[version,setVersion]=useState(0);
  ...
  return <aside className={`artifact-panel ${wide?'artifact-wide':''}`} aria-label="File preview"><header><div><FileText size={16}/><strong title={preview.name}>{preview.name}</strong></div><div>
    ...
    <div className="preview-content">{error?<p role="alert">{error}</p>:mode==='source'||preview.kind==='code'||preview.kind==='text'?<pre className="source-view">{text}</pre>:html?<iframe key={version+text} title={`Preview of ${preview.name}`} sandbox="allow-scripts" referrerPolicy="no-referrer" srcDoc={sandboxDocument(text,preview.kind)}/>:preview.kind==='markdown'?<div className="markdown preview-markdown"><RichMarkdown text={text}/></div>:preview.kind==='image'?<div className="image-preview"><img src={preview.url} alt={preview.name}/></div>:preview.kind==='audio'?<div className="audio-preview"><FileText size={36}/><h3>{preview.name}</h3><audio controls src={preview.url} preload="metadata"/><p>Audio playback · model input requires a transcript</p></div>:preview.kind==='pdf'&&preview.url?<Suspense fallback={<p>Opening PDF…</p>}><PdfPreview url={preview.url}/></Suspense>:null}</div>
    <footer>{html?'Isolated preview · external connections blocked':preview.kind==='pdf'?'Document preview · original file':'Local preview'}<span>{mode==='source'?'Source':preview.kind.toUpperCase()}</span></footer>
  </aside>;
}

    ...
    {visible.map(profile=>{
      ...
      return <article key={profile.id} data-profile-id={profile.id} title={model?.name||profile.name} className={`compact-profile ${open?'expanded':''} ${props.editingId===profile.id?'selected':''} ${state?`live ${state}`:''} ${over===profile.id&&over!==dragged?'drop-target':''} ${dragged===profile.id?'dragging':''}`}>
        ...
        {open&&<div className="compact-profile-details" id={`profile-${profile.id}`}>
          ...
          <div className="profile-detail-grid"><div><span>Context / cache</span><strong>{s.context/1024}K · {s.kv}</strong></div><div><span>Prediction</span><strong>{s.prediction==='mtp'?`MTP on · ${s.draft_tokens} draft tokens`:'MTP off'}</strong></div><div><span>Vision</span><strong>{s.vision?'On':'Off'}</strong></div>{model?.ngram&&<div><span>PLE n-gram table</span><strong>{s.ngram_ram===false?'Storage streaming':'System RAM'}</strong></div>}<div><span>CPU placement</span><strong>{s.cpu_percent}% · {s.cpu_threads} threads{s.engine==='gguf'||model?.format==='GGUF'?` · ${s.gguf_offload}`:''}</strong></div><div><span>Generation</span><strong>{s.max_output?`${s.max_output/1024}K`:'Auto'} output · temp {s.temperature}</strong></div><div><span>Thinking / chunk</span><strong>{reasoningLabel(s.reasoning_effort,model)} · {s.chunk_size} tokens</strong></div></div>
